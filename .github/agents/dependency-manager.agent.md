...
## Project Knowledge
- **Package Manager:** Go modules (for tests); Terraform provider registry (for providers)
- **Dependency Manifest:** `test/go.mod` (Go test deps), `versions.tf` (Terraform providers)
- **Lockfile:** `test/go.sum`, `.terraform.lock.hcl`
- **Audit Command:** `go vuln check` (for Go test deps)
